---
title: Installation
description: Install the uxd CLI globally, from a clone, or run it without installing.
navigation:
  icon: i-lucide-download
seo:
  title: Installation
  description: Requirements and install methods for the uxd CLI — global package manager install, install from a clone, or run the built entrypoint directly.
---

## Requirements

| Tool | Version | Required |
|---|---|---|
| [Node.js](https://nodejs.org) | ≥ 18 | Yes |
| `git` | ≥ 2.38 | Yes |
| [`gh`](https://cli.github.com) | any | No — enables PR metadata, `gh pr diff`, and fork push-back |

`uxd` runs on stock Node. No Bun, Deno, or other runtime is required, and any
package manager installs it.

::note
Every `gh` feature degrades gracefully when `gh` is missing or unauthenticated.
PR checkout, `diff`, and `clean` all fall back to plain git.
::

## Global install

`uxd` publishes to npm as **`@uxfront/uxd`** and installs a `uxd` binary onto
your `PATH`. This is the recommended install.

:::code-group

```bash [npm]
npm i -g @uxfront/uxd
```

```bash [pnpm]
pnpm add -g @uxfront/uxd
```

```bash [yarn]
yarn global add @uxfront/uxd
```

:::

Confirm it landed:

```bash
uxd version
```

```
uxd 1.1.0
```

::tip{title="uxd: command not found"}
Your package manager's global bin directory is not on your `PATH`. Add
`$(npm prefix -g)/bin` (npm), `$(pnpm bin -g)` (pnpm), or `$(yarn global bin)`
(yarn) to it.
::

::warning{title="Upgrading from 1.0.0"}
1.0.0 has no `uxd setup` command — it fails with
`error(E_CONFIG): unknown project 'setup'` — and reports its own version as
`uxd 0.0.0`. 1.1.0 also removed the `extends` project key. Read the
[changelog](https://github.com/uxfront-com/uxd/blob/main/CHANGELOG.md) before
you upgrade.
::

## Install from a clone

Working on `uxd` itself? `uxd` ships as compiled JavaScript, so a clone install
has two steps — install dependencies, then build the `dist/` output the CLI
runs from — before you link it onto your `PATH`.

::steps{level="3"}

### Clone and build

```bash
git clone git@github.com:uxfront-com/uxd.git
cd uxd
npm install
npm run build           # emits dist/bin/uxd.js (the linked bin points here)
```

### Link it onto your PATH

:::code-group

```bash [npm]
npm link
```

```bash [pnpm]
pnpm link --global      # run `pnpm setup` once first
```

```bash [yarn]
yarn global add "file:$PWD"
```

:::

### Verify

```bash
uxd version
```

::

::warning{title="Rebuild after pulling"}
`npm link` points `uxd` at your clone's `dist/`, and the CLI runs compiled
output. Re-run `npm run build` after every `git pull` or your changes will not
take effect.
::

## Run without installing

Skip the global step and call the built entrypoint from the clone:

```bash
npm install && npm run build
node dist/bin/uxd.js version
```

The rest of these docs write `uxd`. Substitute `node dist/bin/uxd.js` if you
have not linked it globally.

## Next

Head to the [quick start](/docs/uxd/getting-started/quick-start) to configure your
first project and materialize a workspace.
